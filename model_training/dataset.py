    ...
    def create_batch_index_train(self):
        ...
        for batch_idx in range(self.n_batches):
            ...
            if self.must_include_days is not None and len(self.must_include_days) > 0:
                ...
                days = np.concatenate((self.must_include_days, np.random.choice(non_must_include_days,
                                                                                size=self.days_per_batch - len(
            ...
            num_trials = math.ceil(
                self.batch_size / self.days_per_batch)  # Use ceiling to make sure we get at least batch_size trials
...
def train_test_split_indicies(file_paths, test_percentage=0.1, seed=-1, bad_trials_dict=None):
    ...
    for i, path in enumerate(file_paths):
        ...
        trials_per_day[i] = {'num_trials': len(good_trial_indices), 'trial_indices': good_trial_indices,
                             'session_path': path}
